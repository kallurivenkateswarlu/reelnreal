...
export default function App() {
  ...
  const filteredMovies = useMemo(() => {
    return siteContent.movies.filter(m => {
  ...
  }, [siteContent.movies, selectedCategory, searchQuery, language]);
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {loading ? (
          ...
          <>
            ...
            {activeTab === 'boxoffice' && (
              <BoxOfficeSection
                items={siteContent.boxOfficeLive}
                top5={siteContent.boxOfficeTop5}
                updates={siteContent.boxOfficeUpdates}
                language={language}
              />
            )}
            ...
            {activeTab === 'news' && (
              <NewsSection
                politicalNews={siteContent.politicalNews}
                localNews={siteContent.localNews}
                language={language}
                onToggleBookmark={handleToggleBookmark}
            ...
            {activeTab === 'history' && (
              <TeluguHistorySection
                historyItems={siteContent.teluguFilmHistory}
                language={language}
                onSelectMovie={setSelectedMovie}
              />
            )}
