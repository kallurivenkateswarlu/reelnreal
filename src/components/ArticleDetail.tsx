...
import { ArrowLeft, ArrowRight, Calendar, ChevronRight, Facebook, Link as LinkIcon, Mail, Share2, Twitter } from 'lucide-react';
...
export const ArticleDetail: React.FC<ArticleDetailProps> = ({ article, related, language, onOpenArticle, onNavigate }) => {
  ...
  const share = async (kind?: string) => {
    ...
    if (kind === 'twitter') window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(title)}&url=${encodeURIComponent(url)}`, '_blank', 'noopener,noreferrer');
    else if (kind === 'facebook') window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}`, '_blank', 'noopener,noreferrer');
    else if (navigator.share) await navigator.share({ title, text: article.excerpt, url }).catch(() => {});
  ...
  return <article className="max-w-5xl mx-auto animate-fade-in">
    ...
    <div className="flex flex-wrap gap-2 mb-8"><button onClick={() => share()} className="share-button"><Share2 className="w-4 h-4" /> {copied ? 'Copied' : 'Share'}</button><button onClick={() => share('twitter')} className="share-button"><Twitter className="w-4 h-4" /> X</button><button onClick={() => share('facebook')} className="share-button"><Facebook className="w-4 h-4" /> Facebook</button><button onClick={() => share()} className="share-button"><Mail className="w-4 h-4" /> Email</button><button onClick={() => share()} className="share-button"><LinkIcon className="w-4 h-4" /> Copy link</button></div>
